...
const TripAccordion = ({ trip }) => {
	...
	const findDepartingCarrier = carriers.find(
		(carrier) => carrier.CarrierId === trip.flight.OutboundLeg.CarrierId
	);
	const departingCarrierLogo = findDepartingCarrier.LogoUrl;
	const departingCarrierName = findDepartingCarrier.Name;
	const findReturningCarrier = trip.flight.InboundLeg
		? carriers.find(
				(carrier) => carrier.CarrierId === trip.flight.InboundLeg.CarrierId
		  )
		: null;
	const returningCarrierLogo = trip.flight.InboundLeg
		? findReturningCarrier.LogoUrl
		: null;
	const returningCarrierName = trip.flight.InboundLeg
				...
				<Grid item container xs={12} alignItems="center">
					<Grid item container xs={8} md={3} lg={4} alignItems="center">
						...
						<Grid item xs={8} className={classes.departTime}>
							<Typography variant="subtitle2" className={classes.departHeader}>
								{`${trip.flight.OutboundLeg.DepartureTime} - ${trip.flight.OutboundLeg.ArrivalTime}`}
							</Typography>
					...
					>
						...
						<Grid item xs={12}>
							...
							>{`${trip.flight.From} - ${trip.flight.To}`}</Typography>
						</Grid>
					</Grid>
