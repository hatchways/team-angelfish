...
const Itinerary = () => {
	...
	const getTrips = async () => {
		try {
			const response = await fetch(`/api/itinerary/${user._id}`);
			const data = await response.json();
			if (response.status === 200) {
				const pastTrips = data.filter(isPast);
				const upcomingTrips = data.filter(isUpcoming);
				setPast(sortTrips(pastTrips));
				setUpcoming(sortTrips(upcomingTrips));
			}
		} catch (err) {
			console.error(err);
		}
